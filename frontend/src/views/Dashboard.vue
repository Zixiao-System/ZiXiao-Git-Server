  ...
  <div class="dashboard-layout">
    ...
    <div class="content-area">
      ...
      <div class="stats-grid">
        <mdui-card class="stat-card">
          <mdui-icon name="folder" style="font-size: 48px; color: var(--mdui-color-primary);"></mdui-icon>
          <div class="stat-value">{{ repositories.length }}</div>
          <div class="stat-label">仓库数量</div>
        </mdui-card>

        <mdui-card class="stat-card">
          <mdui-icon name="people" style="font-size: 48px; color: var(--mdui-color-secondary);"></mdui-icon>
          <div class="stat-value">0</div>
          <div class="stat-label">协作者</div>
        </mdui-card>

        <mdui-card class="stat-card">
          <mdui-icon name="trending_up" style="font-size: 48px; color: var(--mdui-color-tertiary);"></mdui-icon>
          <div class="stat-value">0</div>
      ...
      <h2>最近的仓库</h2>
      <div class="repo-list" v-if="repositories.length > 0">
        <mdui-card v-for="repo in repositories.slice(0, 5)" :key="repo.id" class="repo-card">
          <div class="repo-info">
          ...
          <mdui-button variant="text" @click="$router.push(`/repositories/${repo.id}`)">
            查看详情
          </mdui-button>
        </mdui-card>
      </div>
      <mdui-card v-else style="padding: 32px; text-align: center;">
        <p>还没有仓库，创建第一个仓库吧！</p>
      </mdui-card>
    </div>
    ...
    <mdui-dialog :open="showCreateDialog" @close="showCreateDialog = false">
      ...
      <mdui-dialog-actions>
        ...
        <mdui-button variant="filled" @click="handleCreateRepo">创建</mdui-button>
      </mdui-dialog-actions>
    </mdui-dialog>
  </div>
</template>
